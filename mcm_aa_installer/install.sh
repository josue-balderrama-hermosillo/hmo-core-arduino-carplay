...
if [ "$1" != "--continue-bg" ]; then

echo ">>> Freeing potential dpkg/apt background locks..."
systemctl stop unattended-upgrades apt-daily.service apt-daily.timer apt-daily-upgrade.timer apt-daily-upgrade.service 2>/dev/null || true
while fuser /var/lib/dpkg/lock-frontend >/dev/null 2>&1 || fuser /var/lib/dpkg/lock >/dev/null 2>&1; do
    echo "Wait for dpkg lock..."
    sleep 2
done
...
if [ "$1" != "--continue-bg" ]; then
    ...
    exec systemd-run --working-directory="$PWD" --remain-after-exit bash "$(realpath "$0")" --continue-bg
fi
